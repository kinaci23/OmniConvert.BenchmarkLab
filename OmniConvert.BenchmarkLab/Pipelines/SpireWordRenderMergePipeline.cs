...
public sealed class SpireWordRenderMergePipeline : IConversionPipeline
{
    public string Name => "SpireWordRenderMergePipeline";
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            document.LoadFromFile(request.InputPath);

            renderedImages = document.SaveToImages(ImageType.Bitmap);

            if (renderedImages == null || renderedImages.Length == 0)
            {
                throw new InvalidOperationException("Spire hiçbir sayfa render edemedi.");
            }
            ...
            firstFrame = PrepareFrame(renderedImages[0], request.Profile);
            ...
            for (int i = 1; i < renderedImages.Length; i++)
            {
                ...
                using Image nextFrame = PrepareFrame(renderedImages[i], request.Profile);
